...
/**
 * This class implements the PortfolioList Interface. This class has a list of Flexible Portfolios
 * that are created.
 */
public class FlexiblePortfolioListImpl extends AbstractPortfolioListImpl
  ...
  @Override
  public boolean invalidateTransactions(String pName, LocalDate date) {
    FlexiblePortfolio p = getPortfolio(pName);
    return p.invalidateTransaction(date);
  }

  @Override
  public void rebalancePortfolio(String pName, Map<String, Integer> percentageMappings,
      LocalDate date,
      Map<String, Float> quantityMapping, PortfolioItem[] portfolioItems) {
    Map<String, Float> valuePerStockMappings = new HashMap<>();

    Float totalValueAtDate = this.getPortfolioValueAtDate(pName, date);

    for (PortfolioItem p : portfolioItems) {
      valuePerStockMappings.put(p.getStock().getTicker(),
          p.getStock().getCurrentPriceAtDate(date));
    }

    Set<String> uniqueSet = new HashSet<>();

    for (PortfolioItem p : portfolioItems) {
      uniqueSet.add(p.getStock().getTicker());
    }

    for (Map.Entry<String, Integer> entry : percentageMappings.entrySet()) {
      Float actualValue = (totalValueAtDate * entry.getValue()) / 100;
      Float actualQuantity = actualValue / valuePerStockMappings.get(entry.getKey());
      if (actualQuantity < quantityMapping.get(entry.getKey())) {
        Float quantityToSell = Math.abs(actualQuantity - quantityMapping.get(entry.getKey()));
        this.addTransactionToPortfolio(pName, TransactionType.SELL, entry.getKey(),
            quantityToSell, date, 0);
      } else if (actualQuantity > quantityMapping.get(entry.getKey())) {
        Float quantityToBuy = actualQuantity - quantityMapping.get(entry.getKey());
        this.addTransactionToPortfolio(pName, TransactionType.BUY, entry.getKey(),
            quantityToBuy, date, 0);
      }
    }
  }
