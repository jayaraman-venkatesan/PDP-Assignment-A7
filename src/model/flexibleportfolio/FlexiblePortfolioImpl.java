...
public class FlexiblePortfolioImpl extends PortfolioImpl implements FlexiblePortfolio {
  ...
  /**
   * This method constructs the FlexiblePortfolio Object based on the given name and empty portfolio
   * Items.
   *
   ...
   * @param fileName                  the filename that this portfolio already has. Can be null to
   *                                  auto assign.
  ...
  @Override
  public Boolean invalidateTransaction(LocalDate date) {

    Map<String, Float> quantityMapping = new HashMap<>();



//    for (PortfolioItemTransaction p : this.portfolioItemTransactions) {
//      if (!p.getDate().isBefore(date)) {
//        if (!quantityMapping.containsKey(p.getStock().getTicker())) {
//          quantityMapping.put(p.getStock().getTicker(), p.getQuantity());
//        } else {
//          if (p.getType().equals(TransactionType.BUY)) {
//            quantityMapping.put(p.getStock().getTicker(),
//                quantityMapping.get(p.getStock().getTicker()) + p.getQuantity());
//          }
//          if(p.getType().equals(TransactionType.SELL)){
//            Float availableQuantity  = quantityMapping.get(p)
//          }
//        }
//      }
//    }
